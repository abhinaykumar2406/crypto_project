...
char* create_login_attempt(const char *username) {
    ...
    memset(&attempt, 0, sizeof(LoginAttempt));
...
int verify_token1(const char *username, const char *token) {
    ...
    if (!file) {
        printf("[Server] No login attempts file found.\n");
        return 0;
    }
    ...
    printf("\n[Server] Verifying Token1 for username: '%s', token: '%s'\n", username, token);

    while (fgets(line, sizeof(line), file)) {
        LoginAttempt attempt;
        sscanf(line, "%s %ld %d %d %s %s",
               attempt.username, &attempt.timestamp,
               &attempt.halfway_confirmed, &attempt.zkp_passed,
               attempt.token1, attempt.token2);

        printf("[Server] Checking stored attempt:\n"
               "  username='%s'\n  token1='%s'\n  age=%ld seconds\n",
               attempt.username, attempt.token1, current_time - attempt.timestamp);
        ...
        if (strcmp(attempt.username, username) == 0 &&
            ...
            printf("[Server] ✅ Token1 verified successfully for '%s'\n", username);
        }
    ...
    if (!token_valid)
        printf("[Server] ❌ Token1 verification failed.\n");
...
int verify_token21(const char *username, const char *token) {
    FILE *file = fopen(LOGIN_ATTEMPTS_FILE, "r");
    if (!file) return 0;
    char line[MAX_LINE];
    time_t current_time = time(NULL);
...
int verify_token2(const char *username, const char *token) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        sscanf(line, "%49s %ld %d %d %16s %16s", attempt.username, &attempt.timestamp,
               &attempt.halfway_confirmed, &attempt.zkp_passed, attempt.token1, attempt.token2);

        // Remove possible trailing newline from token2
        attempt.token2[strcspn(attempt.token2, "\n")] = 0;

        if (strcmp(attempt.username, username) == 0 &&
            (current_time - attempt.timestamp) < LOGIN_EXPIRY_SECONDS &&
            attempt.zkp_passed == 1 &&
            strcmp(attempt.token2, token) == 0) {
            // Token2 is valid, login completed: do NOT write it back
            token_valid = 1;
            printf("[Server] ✅ Token2 verified successfully for user: %s\n", username);
        } else {
            // Keep the record in the temp file
            fprintf(temp_file, "%s", line);
        }
    }
