...
int verify_password_with_server(const char *username, const char *password, EVP_PKEY *device_key) {
    char message[512];
    snprintf(message, sizeof(message), "ACTION: VERIFY_PASSWORD\nUsername: %s\nPassword: %s\n", username, password);
    size_t sig_len;
    unsigned char *signature = sign_message(device_key, (const unsigned char*)message, strlen(message), &sig_len);
    ...
    if (strstr(response, "Password OK") != NULL) {
        return 1; // Password correct
    } else {
        return 0; // Incorrect password
    }
}
// Check password locally using stored secret key
int check_password_local(const char *username, const char *password) {
    FILE *keyfile = fopen("device_keys.txt", "r");
    if (!keyfile) return 0;

    char line[1024];
    int found = 0;
    BIGNUM *stored_secret = BN_new();
    BIGNUM *p = BN_new(), *g = BN_new();
    while (fgets(line, sizeof(line), keyfile)) {
        char stored_username[50], secret1_hex[256], secret2_hex[256], p_hex[256], g_hex[256];
        sscanf(line, "%s %s %s %s %s", stored_username, secret1_hex, secret2_hex, p_hex, g_hex);
        if (strcmp(stored_username, username) == 0) {
            BN_hex2bn(&stored_secret, secret1_hex);
            BN_hex2bn(&p, p_hex);
            BN_hex2bn(&g, g_hex);
            found = 1;
            break;
    ...
    fclose(keyfile);
    if (!found) {
        BN_free(stored_secret);
        BN_free(p); BN_free(g);
        return 0;
    }

    // Convert entered password to secret key
    BIGNUM *entered_secret = password_to_secret_key(password);

    int match = BN_cmp(stored_secret, entered_secret) == 0;

    BN_free(stored_secret);
    BN_free(entered_secret);
    BN_free(p); BN_free(g);
...
int main() {
    ...
    if (choice == 1) {
        ...
        fgets(password, sizeof(password), stdin);
        password[strcspn(password, "\n")] = 0; // Remove newline
        // Step 0: Verify password first
        //if (!verify_password_with_server(username, password, device_key)) {
        // printf("Error: Invalid password. Cannot initiate login.\n");
        // return 1;
        //}
        // Check password first
        if (!check_password_local(username, password)) {
         printf("Error: Invalid password. Cannot initiate login.\n");
         return 1;
        }
